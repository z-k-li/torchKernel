...
import torch
# def get_torch_subset(yt,view, sinogram_template):
#     y=sinogram_template.copy().fill(yt.detach().cpu().numpy())
#     ys=y.get_subset(view)
#     return torch.tensor(ys.as_array())
...
class _primal_op(torch.autograd.Function):
    ...
    def forward(ctx, x, image_template, sinogram_template, sirf_obj):
        ...
        if(image_template.dimensions().__len__()==x_np.ndim):
            x_sirf = ctx.image_template.copy().fill(x_np)
        else:
            x_sirf = ctx.image_template.fill(x_np[0,...])

        proj_sinogram_np = ctx.sirf_obj.forward(x_sirf).as_array()
...
class primal_op(torch.nn.Module):
    ...
    def forward(self, image):
        ...
        # if (image.shape[1]>1):
        #     image_nc_flat = image.view(-1, *image.shape[1:])
        # else:
        ...
        num_unsqueeze = image.dim()-sym_sinogram_i.dim()
        sinogram = torch.cat(sinogram_nc_flat)
        for i in range(num_unsqueeze): 
            sinogram.unsqueeze_(dim=0)
        return sinogram
...
class _dual_op(torch.autograd.Function):
    ...
    def forward(ctx, sinogram, image_template, sinogram_template, sirf_obj):
        ...
        if(sinogram_template.dimensions().__len__()==sinogram_np.ndim):
            sinogram_np = ctx.sinogram_template.copy().fill(sinogram_np)
        else:
            sinogram_np = ctx.sinogram_template.copy().fill(sinogram_np[0,...])
    ...
    def backward(ctx, x):
        x_np = x.detach().cpu().numpy()
        x_np = ctx.image_template.copy().fill(x_np)
        proj_sinogram_np = ctx.sirf_obj.forward(x_np).as_array()
        proj_sinogram = torch.from_numpy(proj_sinogram_np).requires_grad_().to(x.device)
        num_unsqueeze = x.dim()-proj_sinogram.dim()
        
        for i in range(num_unsqueeze):    
            proj_sinogram.unsqueeze_(dim=0)
...
class dual_op(torch.nn.Module):
    ...
    def forward(self, sinogram):
        # x.shape: (N, C, H, W) or (N, C, D, H, W)
        # if (sinogram.shape[1]>1):
        #     sinogram_nc_flat = sinogram.view(-1, *sinogram.shape[1:])
        # else:
        ...
        for x_i in sinogram_nc_flat:
            ...
            #     sym_image_i = _dual_op.apply(x_i[None], self.image_template, self.sinogram_template, self.acq_model)
            # else:
        ...
        num_unsqueeze = sinogram.dim()-sym_image_i.dim()
        image = torch.cat(image_nc_flat)
        for i in range(num_unsqueeze):    
